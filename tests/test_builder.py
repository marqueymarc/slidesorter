...
class BuilderTests(unittest.TestCase):
    def test_lazy_build_creates_external_state_and_catalog(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("/gallery/app.js?v=3.9.5", index)
            self.assertIn("/gallery/appearance.js?v=3.9.5", index)
            ...
            self.assertIn('document.querySelectorAll(".card").forEach(candidate => candidate.classList.toggle("shortcut-target", candidate === target))', app)
            self.assertIn('document.activeElement?.closest?.(".card")', app)
            self.assertNotIn("reconcileHoverIntent", app)
            self.assertNotIn("single-destination-actions", app)
            viewer = (state / "viewer.html").read_text()
            self.assertIn('id="viewer-undo"', viewer)
            self.assertIn('event.code === "KeyU"', viewer)
            self.assertIn('state.lastActionToken', viewer)
            self.assertIn('navigate(advanceTo, true)', viewer)
            self.assertIn('if (event.key.length !== 1 || event.metaKey', viewer)
